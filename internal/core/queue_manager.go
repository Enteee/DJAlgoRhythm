...
func (d *Dispatcher) fillQueueToTargetDuration(ctx context.Context, targetDuration, currentDuration time.Duration) {
	...
	d.sendQueueTrackApprovalMessage(ctx, trackID, track, messageKey, "queue management track", autoApprove, searchQuery, newTrackMood)
...
func (d *Dispatcher) findAndSuggestReplacementTrack(ctx context.Context) {
	...
	d.sendQueueTrackApprovalMessage(ctx, newTrackID, track, messageKey, "replacement queue track", autoApprove, newSearchQuery, newTrackMood)
