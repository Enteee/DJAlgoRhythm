...
// sendDeviceWarningToAdmins sends device warning messages to all admin users via DM
func (d *Dispatcher) sendDeviceWarningToAdmins(ctx context.Context, adminUserIDs []string) {
	message := d.localizer.T("admin.no_active_device")

	successCount := 0
	var errors []string

	// Track message IDs for later deletion
	d.deviceWarningMutex.Lock()
	for _, adminUserID := range adminUserIDs {
		msgID, err := d.frontend.SendDirectMessage(ctx, adminUserID, message)
		if err != nil {
			d.logger.Warn("Failed to send device warning to admin",
				zap.String("adminUserID", adminUserID),
				zap.Error(err))
			errors = append(errors, err.Error())
		} else {
			d.deviceWarningMessages[adminUserID] = msgID
			successCount++
		}
	}
	d.deviceWarningMutex.Unlock()

	d.logger.Info("Device warning sent to admins",
		zap.Int("successCount", successCount),
		zap.Int("totalAdmins", len(adminUserIDs)),
		zap.Strings("errors", errors))
}

// shouldSendDeviceWarning checks if device warning should be sent (only if not already active)
func (d *Dispatcher) shouldSendDeviceWarning() bool {
	d.deviceWarningMutex.RLock()
	warningActive := d.deviceWarningActive
	d.deviceWarningMutex.RUnlock()

	return !warningActive // Send warning only if no warning is currently active
}

// markDeviceWarningSent marks that a device warning has been sent
func (d *Dispatcher) markDeviceWarningSent() {
	d.deviceWarningMutex.Lock()
	d.deviceWarningActive = true
	d.deviceWarningMutex.Unlock()
}

// clearDeviceWarning clears the device warning state and deletes sent messages when device becomes active
func (d *Dispatcher) clearDeviceWarning(ctx context.Context) {
	d.deviceWarningMutex.Lock()
	wasActive := d.deviceWarningActive
	messagesToDelete := make(map[string]string)

	// Copy messages to delete before clearing
	for userID, msgID := range d.deviceWarningMessages {
		messagesToDelete[userID] = msgID
	}

	// Clear the state
	d.deviceWarningActive = false
	d.deviceWarningMessages = make(map[string]string)
	d.deviceWarningMutex.Unlock()

	if wasActive {
		d.logger.Info("Device warning cleared - active Spotify device detected")

		// Delete the warning messages from admin DMs
		deletedCount := 0
		var deleteErrors []string
		for userID, msgID := range messagesToDelete {
			if err := d.frontend.DeleteMessage(ctx, userID, msgID); err != nil {
				d.logger.Warn("Failed to delete device warning message",
		...
		d.logger.Info("Device warning messages deleted",
			zap.Int("deletedCount", deletedCount),
			zap.Int("totalMessages", len(messagesToDelete)),
			zap.Strings("deleteErrors", deleteErrors))
	}
}
