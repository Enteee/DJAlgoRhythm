...
const (
	...
	defaultAdminConfirmTimeoutSecs        = 3600
	...
	shutdownGracePeriod                   = shutdownTimeoutSecs * time.Second
	gracefulShutdownDelay                 = 2
	envExampleFilePermissions             = 0600
)
...
//nolint:gochecknoinits // Required by Cobra CLI framework for flag registration
func init() {
...
func runServices(ctx context.Context, svcs *services) error {
	...
	if err := g.Wait(); err != nil {
		...
		if stopErr := svcs.dispatcher.Stop(shutdownCtx); stopErr != nil {
	...
	shutdownCtx, shutdownCancel := context.WithTimeout(context.WithoutCancel(ctx), shutdownGracePeriod)
	defer shutdownCancel()
	if err := svcs.dispatcher.Stop(shutdownCtx); err != nil {
...
func promptForTelegramGroup() (int64, error) {
	...
	time.Sleep(gracefulShutdownDelay * time.Second)
...
func validateSpotifyConfig() error {
	...
	if config.Spotify.ClientSecret == "" {
		return errors.New("spotify client secret is required")
	}
...
func validateLLMConfig() error {
	...
	if config.LLM.Provider == "" || config.LLM.Provider == noneProvider {
		return errors.New("AI provider is required - please configure one of: openai, anthropic, ollama")
	}
	...
	if !isValid {
		return fmt.Errorf("unsupported AI provider '%s' - supported providers: %s",
			config.LLM.Provider, strings.Join(validProviders, ", "))
	}
...
func generateEnvExample(cmd *cobra.Command) error {
	...
	if err := os.WriteFile(".env.example", []byte(content), envExampleFilePermissions); err != nil {
...
func generateTelegramSection(content *strings.Builder, cmd *cobra.Command) {
	...
	fmt.Fprintf(content, "## Require approval even from admins - for testing (default: %s)\n", adminDefault)
...
func generateSpotifySection(content *strings.Builder, _ *cobra.Command) {
	...
	fmt.Fprintf(content, "%s=your_spotify_client_id_here\n", flagToEnvVar("spotify-client-id"))
	content.WriteString("## Spotify app client secret\n")
...
func generateLLMSection(content *strings.Builder, _ *cobra.Command) {
	...
	content.WriteString("## Provider: openai, anthropic, ollama (REQUIRED)\n")
	fmt.Fprintf(content, "%s=openai\n", flagToEnvVar("llm-provider"))
	content.WriteString("\n")
	content.WriteString("## -----------------------------------------------------------------------------\n")
	...
	fmt.Fprintf(content, "# %s=sk-...\n", flagToEnvVar("llm-api-key"))
	content.WriteString("## Model name (gpt-4o-mini is cost-effective)\n")
	fmt.Fprintf(content, "# %s=gpt-4o-mini\n", flagToEnvVar("llm-model"))
	content.WriteString("\n")
	content.WriteString("## -----------------------------------------------------------------------------\n")
	...
	fmt.Fprintf(content, "# %s=sk-ant-...\n", flagToEnvVar("llm-api-key"))
	content.WriteString("## Model name (Haiku is fastest/cheapest)\n")
	fmt.Fprintf(content, "# %s=claude-3-haiku-20240307\n", flagToEnvVar("llm-model"))
	content.WriteString("\n")
	content.WriteString("## -----------------------------------------------------------------------------\n")
	...
	content.WriteString("## Ollama server URL (default: http://localhost:11434)\n")
	fmt.Fprintf(content, "# %s=http://localhost:11434\n", flagToEnvVar("llm-base-url"))
...
func generateAppLocalizationSection(content *strings.Builder, cmd *cobra.Command) {
	...
	fmt.Fprintf(content, "## Bot language: %s (default: %s)\n", supportedLangs, langDefault)
	fmt.Fprintf(content, "%s=%s\n", flagToEnvVar("language"), langDefault)
	content.WriteString("\n")
}
...
func generateAppTimeoutsSection(content *strings.Builder, cmd *cobra.Command) {
	...
	fmt.Fprintf(content, "%s=%s\n", flagToEnvVar("max-queue-track-replacements"), maxReplacementsDefault)
	content.WriteString("\n")
}
...
func generateServerSection(content *strings.Builder, cmd *cobra.Command) {
	...
	fmt.Fprintf(content, "## Server bind address (default: %s)\n", hostDefault)
	fmt.Fprintf(content, "%s=%s\n", flagToEnvVar("server-host"), "127.0.0.1")
	fmt.Fprintf(content, "## Server port (default: %s)\n", portDefault)
	fmt.Fprintf(content, "%s=%s\n", flagToEnvVar("server-port"), portDefault)
	content.WriteString("\n")
}
