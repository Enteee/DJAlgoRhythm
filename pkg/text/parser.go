...
// Parser provides text parsing functionality for chat messages and URL extraction.
type Parser struct{}

// NewParser creates a new Parser instance for text processing.
func NewParser() *Parser {
	return &Parser{}
}

// ParseMessage parses a text message and classifies it into appropriate message types.
...
// ExtractSpotifyTrackID extracts a Spotify track ID from various URL formats and redirects.
func (p *Parser) ExtractSpotifyTrackID(rawURL string) (string, error) {
	...
	trackID := extractTrackIDFromURLPath(u.Path)
	return trackID, nil
}

// extractTrackIDFromURLPath extracts a Spotify track ID from a URL path.
func extractTrackIDFromURLPath(path string) string {
